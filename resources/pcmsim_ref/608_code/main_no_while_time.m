...
[delta_H,delta_H_old,lfrac,initial_state,initial_Temperature,...
    uint,vint,p] = initial_conditions_io(CVx,CVy,latent_heat,b_conditions);
%%
...
%% Write to file 
% Temperature
fileID_temp = fopen('temp.txt','w');
fprintf(fileID_temp,'%f \n',Temperature);
fclose(fileID_temp);
% U
fileID_u = fopen('u.txt','w');
fprintf(fileID_u,'%f \n',u);
fclose(fileID_u);
% V
fileID_v = fopen('v.txt','w');
fprintf(fileID_v,'%f \n',v);
fclose(fileID_v);
% Pressure
fileID_p = fopen('p.txt','w');
fprintf(fileID_p,'%f \n',p);
fclose(fileID_p);
