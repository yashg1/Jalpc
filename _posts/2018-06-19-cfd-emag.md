...
## Motivation
{: .alert .alert-info}
 Traditional die casting (which uses molten metal) has quality issues due to porosity, shrinkage, gas entrainment because of turbulent die filling. Semisolid metals processing (SSM) uses a semisolid slurry for casting. The slurry is maintained between the liquidus and solidus temperatures.    The *high viscosity* of the semisolid material eliminates these defects because of controlled die filling. 
